...
@Component({
  selector: 'app-pmassets',
  templateUrl: './pmassets.component.html',
  styleUrls: ['./pmassets.component.css']
})
export class PmassetsComponent implements OnInit {
  ...
  generatePMPDF() {
    ...
    if (this.lstCheckedAssets.length == 0) {
      ...
      this.assetDetailService.CreateAssetCheckedPMPDFByDepartment(this.sortFilterObjects).subscribe(list => {
      ...
      this.assetDetailService.CreateAssetCheckedPMPDF(this.lstCheckedAssets).subscribe(list => {
        var filePath = `${environment.Domain}UploadedAttachments/`;
        let fileName = "WOPM2.pdf";
        this.uploadService.downloadWOPMPDF(fileName).subscribe(file => {
          var dwnldFile = filePath + 'WOReports/' + fileName;
